CREATE TABLE `bayserhonduras`.`productos` (
...
CREATE TABLE `bayserhonduras`.`carretilla` (
  ...
  CONSTRAINT `carretilla_user_key`
    FOREIGN KEY (`usercod`)
    REFERENCES `bayserhonduras`.`usuario` (`usercod`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `carretilla_prd_key`
    FOREIGN KEY (`codprd`)
    REFERENCES `bayserhonduras`.`productos` (`codprd`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
